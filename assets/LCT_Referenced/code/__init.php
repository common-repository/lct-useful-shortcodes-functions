	...
	class lct {
		...
		function init() {
			...
			$this->settings = [
				...
				//ACF
				'acf_dev'                             => false,
				'acf_current_form'                    => null,
				'acf_form'                            => 'lct_acf_form_',
				'acf_form_rand'                       => 'lct_acf_form_rand_',
				'acf_post_id'                         => 'lct_acf_post_id',
		...
		function login_bypass() {
			if (
				get_option( 'whl_page' ) &&
				is_user_logged_in() &&
				$_SERVER['REQUEST_URI'] == '/' . get_option( 'whl_page' ) . '/'
			) {
		...
		function load_plugin_extensions() {
			...
			if ( lct_plugin_active( $plugin ) ) {
				...
				if ( version_compare( lct_plugin_version( $plugin ), '5.5', '<' ) ) //ACF older than v5.5
					lct_load_class( "{$dir}/termmeta.php", 'termmeta', [ 'plugin' => $plugin ] );
			 ...
			 */
			$plugin = 'baw-force-plugin-updates';
			$dir    = "plugins/{$plugin}";

			if ( ! defined( 'DISABLE_BAW' ) ) //We may want to disable this sometimes
				lct_include( "{$dir}/index.php" );
