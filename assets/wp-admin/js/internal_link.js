...
(function( lctinternallinkzxzpShortCodeUI, $ ) {

	var sui = window.lctinternallinkShortcodeUI = {
		models: {},
		collections: {},
		views: {},
		utils: {},
	...
	sui.models.Shortcode = Backbone.Model.extend( {
		...
		validate: function( shortcode ) {
			var errors = [];
			var lct_id = shortcode.attrs.findWhere( { attr: 'id' } );
			if( !lct_id.get( 'value' ) ) {
				errors.push( { 'id': sui.strings.pleaseEnterAnID } );
			}
	...
	sui.views.editShortcodeForm = wp.Backbone.View.extend( {
		...
		el: '#lct_internal_link-shortcode-ui-container',

		template: wp.template( 'lct_internal_link-shortcode-default-edit-form' ),

		hasAdvancedValue: false,

		events: {
			'click #lct_internal_link-update-shortcode': 'insertShortcode',
			'click #lct_internal_link-insert-shortcode': 'insertShortcode',
			'click #lct_internal_link-cancel-shortcode': 'cancelShortcode'
		},
		...
		initialize: function() {
			...
			this.model.get( 'attrs' ).each( function( attr ) {
				switch( attr.get( 'section' ) ) {
					case 'required':
						t.views.add(
							'.lct-edit-shortcode-form-required-attrs',
							new sui.views.editAttributeField( { model: attr, parent: t } )
						);
						break;
					case 'standard':
						t.views.add(
							'.lct-edit-shortcode-form-standard-attrs',
							new sui.views.editAttributeField( { model: attr, parent: t } )
						);
						break;
					default:
		...
		afterRender: function() {
			...
			$( '#lct_internal_link-insert-shortcode' ).toggle( this.options.viewMode == 'insert' );
			$( '#lct_internal_link-update-shortcode' ).toggle( this.options.viewMode != 'insert' );
			$( '#lct-edit-shortcode-form-advanced-attrs' ).toggle( this.hasAdvancedVal );
		},
		...
		dispose: function() {
			this.remove();
			$( '#lct_internal_link-shortcode-ui-wrap' ).append( '<div id="lct_internal_link-shortcode-ui-container"></div>' );
		}
	} );
	...
	sui.views.editAttributeField = Backbone.View.extend( {
		...
		events: {
			...
			'change #lct-shortcode-attr-action': 'updateAction',
		...
		render: function() {
			this.template = wp.media.template( 'lct-shortcode-ui-field-' + this.model.get( 'type' ) );
		...
		updateAction: function( e ) {
			...
			var newShortcodeModel = sui.shortcodes.findWhere( { shortcode_tag: 'lct_internal_link', action_tag: val } );
			...
			lctinternallinkShortcodeUI = new sui.views.editShortcodeForm( {
				model: newShortcodeModel,
				viewMode: viewMode
			} );
			lctinternallinkShortcodeUI.render();
	...
	sui.utils.shortcodeViewConstructor = {
		...
		fetch: function() {
			...
			if( !this.fetching ) {
				...
				$.post( ajaxurl, data ).done( function( response ) {
					...
					self.content = '<span class="lct_shortcode_ui_error">' + lct_internal_linkShortcodeUIData.strings.errorLoadingPreview + '</span>';
				} ).always( function() {
		...
		// Backwards compatibility for WP pre-4.2
		View: {
			overlay: true,
				...
				this.getNodes( function( editor, node, content ) {
					...
					setTimeout( function() {
						iframe = dom.add( content, 'iframe', {
							...
							frameBorder: '0',
							id: 'lct_internal_link-shortcode-preview-' + new Date().getTime(),
		...
		edit: function( shortcodeString ) {
			...
			currentShortcode = wp.shortcode.next( 'lct_internal_link', shortcodeString );
			...
			if( currentShortcode ) {
				...
				$( '#add_lct_internal_link_id' ).val( formId );

				lctinternallinkShortcodeUI = new sui.views.editShortcodeForm( {
					model: currentShortcodeModel,
					viewMode: 'update'
				} );
				lctinternallinkShortcodeUI.render();

				$( '#lct_internal_link-insert-shortcode' ).hide();
				$( '#lct_internal_link-update-shortcode' ).show();
				tb_show( "Edit Page Title", "#TB_inline?inlineId=select_lct_internal_link&width=753&height=686", "" );
	...
	$( document ).ready( function() {
		sui.strings = lct_internal_linkShortcodeUIData.strings;

		sui.shortcodes = new sui.collections.Shortcodes( lct_internal_linkShortcodeUIData.shortcodes );

		if( !lct_internal_linkShortcodeUIData.previewDisabled && typeof wp.mce != 'undefined' ) {
			wp.mce.views.register( 'lct_internal_link', $.extend( true, {}, sui.utils.shortcodeViewConstructor ) );
		}

		$( document ).on( 'click', '.lct_internal_link_media_link', function() {
			sui.shortcodes = new sui.collections.Shortcodes( lct_internal_linkShortcodeUIData.shortcodes );
			var shortcode = sui.shortcodes.findWhere( { shortcode_tag: 'link', action_tag: '' } );
			lctinternallinkShortcodeUI = new sui.views.editShortcodeForm( { model: shortcode, viewMode: 'insert' } );
			lctinternallinkShortcodeUI.render();
			tb_show( "Insert an Internal Link", "#TB_inline?inlineId=select_lct_internal_link&width=753&height=686", "" );
		} );
	} );

}( window.lctinternallinkzxzpShortCodeUI = window.lctinternallinkzxzpShortCodeUI || {}, jQuery ));
