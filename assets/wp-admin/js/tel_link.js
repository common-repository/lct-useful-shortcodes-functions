...
var lcttellinkShortcodeUI;

(function( lcttellinkzxzpShortCodeUI, $ ) {

	var sui = window.lcttellinkShortcodeUI = {
		models: {},
		collections: {},
		views: {},
		utils: {},
	...
	sui.models.Shortcode = Backbone.Model.extend( {
		...
		validate: function( shortcode ) {
			var errors = [];
			var phone = shortcode.attrs.findWhere( { attr: 'phone' } );
			if( !phone.get( 'value' ) ) {
				errors.push( { 'phone': sui.strings.pleaseEnterAPhone } );
			}
	...
	sui.views.editShortcodeForm = wp.Backbone.View.extend( {
		...
		el: '#lct_tel_link-shortcode-ui-container',

		template: wp.template( 'lct_tel_link-shortcode-default-edit-form' ),

		hasAdvancedValue: false,

		events: {
			'click #lct_tel_link-update-shortcode': 'insertShortcode',
			'click #lct_tel_link-insert-shortcode': 'insertShortcode',
			'click #lct_tel_link-cancel-shortcode': 'cancelShortcode'
		},
		...
		afterRender: function() {
			...
			$( '#lct_tel_link-insert-shortcode' ).toggle( this.options.viewMode == 'insert' );
			$( '#lct_tel_link-update-shortcode' ).toggle( this.options.viewMode != 'insert' );
		...
		dispose: function() {
			this.remove();
			$( '#lct_tel_link-shortcode-ui-wrap' ).append( '<div id="lct_tel_link-shortcode-ui-container"></div>' );
		}
	} );
	...
	sui.views.editAttributeField = Backbone.View.extend( {
		...
		updateAction: function( e ) {
			...
			var newShortcodeModel = sui.shortcodes.findWhere( { shortcode_tag: 'lct_tel_link', action_tag: val } );
			...
			lcttellinkShortcodeUI = new sui.views.editShortcodeForm( {
				model: newShortcodeModel,
				viewMode: viewMode
			} );
			lcttellinkShortcodeUI.render();
	...
	sui.utils.shortcodeViewConstructor = {
		...
		fetch: function() {
			...
			if( !this.fetching ) {
				...
				$.post( ajaxurl, data ).done( function( response ) {
					...
					self.content = '<span class="lct_shortcode_ui_error">' + lct_tel_linkShortcodeUIData.strings.errorLoadingPreview + '</span>';
				} ).always( function() {
				...
				this.getNodes( function( editor, node, content ) {
					...
					setTimeout( function() {
						iframe = dom.add( content, 'iframe', {
							...
							frameBorder: '0',
							id: 'lct_tel_link-shortcode-preview-' + new Date().getTime(),
		...
		edit: function( shortcodeString ) {
			...
			currentShortcode = wp.shortcode.next( 'lct_tel_link', shortcodeString );
			...
			if( currentShortcode ) {
				...
				$( '#add_lct_tel_link_id' ).val( formId );
				...
				$( '#lct_tel_link-insert-shortcode' ).hide();
				$( '#lct_tel_link-update-shortcode' ).show();
				tb_show( "Edit Phone Number", "#TB_inline?inlineId=select_lct_tel_link&width=753&height=686", "" );
	...
	$( document ).ready( function() {
		sui.strings = lct_tel_linkShortcodeUIData.strings;

		sui.shortcodes = new sui.collections.Shortcodes( lct_tel_linkShortcodeUIData.shortcodes );

		if( !lct_tel_linkShortcodeUIData.previewDisabled && typeof wp.mce != 'undefined' ) {
			wp.mce.views.register( 'lct_tel_link', $.extend( true, {}, sui.utils.shortcodeViewConstructor ) );
		}

		$( document ).on( 'click', '.lct_tel_link_media_link', function() {
			sui.shortcodes = new sui.collections.Shortcodes( lct_tel_linkShortcodeUIData.shortcodes );
			var shortcode = sui.shortcodes.findWhere( { shortcode_tag: 'lct_tel_link', action_tag: '' } );
			lcttellinkShortcodeUI = new sui.views.editShortcodeForm( { model: shortcode, viewMode: 'insert' } );
			lcttellinkShortcodeUI.render();
			tb_show( "Insert a Phone Number", "#TB_inline?inlineId=select_lct_tel_link&width=753&height=686", "" );
		} );
	} );

}( window.lcttellinkzxzpShortCodeUI = window.lcttellinkzxzpShortCodeUI || {}, jQuery ));
