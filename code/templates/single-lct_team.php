<?php //edit_zz cs - SEE .git ORIGINALzz - themes/Avada/single.php
/**
		...
		<?php while( have_posts() ) : ?>
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'post' ); ?>>
				...
				<div class="post-content">
					<?php
					$full_image = wp_get_attachment_image_src( get_post_thumbnail_id(), get_cnst( 'team' ) );

					if ( $full_image[0] ) {
						echo '<div class="alignleft">';
						echo '<img src="' . $full_image[0] . '"/>';
						echo '</div>';
					}
					?>
					<?php the_content(); ?>
				...
				<?php if ( ! post_password_required( $post->ID ) ) : ?>
					...
					if (
						( $related_posts = lct_fusion_get_custom_posttype_related_posts_team( get_the_ID() ) )
						&& isset( $related_posts )
