<?php
/**
 * `{zxzu}dev_report`: Single word, no spaces. Underscores allowed. e.g. donate_button
 * `Dev Report`: Multiple words, can include spaces, visible when selecting a field type. e.g. Donate Button
 *
 * @since        7.47
 * @verified     2016.12.03
...
if ( ! class_exists( 'acf_field_lct_dev_report' ) ) :


	class acf_field_lct_dev_report extends acf_field
	{
		public $name;
		public $label;
		public $category;
		...
		function __construct()
		{
			 ...
			 */
			$this->name = zxzu( 'dev_report' );

			/**
			 * label (string) Multiple words, can include spaces, visible when selecting a field type
			 */
			$this->label = __( 'Dev Report', 'TD_LCT' );

			/**
			 * category (string) basic | content | choice | relational | jquery | layout | CUSTOM GROUP NAME
			 */
			$this->category = zxzb( ' DEV Check' );
		...
		function render_field( $field )
		{
			 ...
			 * @verified 2021.08.30
			 */
			do_action( 'lct/acf/dev_report', $field );
		}
		 ...
		 *
		 * @unused param mixed $post_id (mixed) the $post_id from which the value was loaded
		 * @unused param array $field   (array) the field array holding all the field options
		 * @type filter
		 * @return mixed $value
		 * @since  3.6
		 * @date   23/01/13
		 ...
		 *
		 * @unused param mixed $value (mixed) the $_POST value
		 * @unused param array $field (array) the field array holding all the field options
		 * @unused param string $input (string) the corresponding input name for $_POST value
		 * @type filter
		 * @return mixed $valid
		 * @date   11/02/2014
		 * @since  5.0.0
	...
	acf_register_field_type( 'acf_field_lct_dev_report' );
