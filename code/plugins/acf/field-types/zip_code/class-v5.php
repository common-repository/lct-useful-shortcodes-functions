<?php
/**
 * `{zxzu}zip_code`: Single word, no spaces. Underscores allowed. e.g. donate_button
 * `Zip Code ({zxza})`: Multiple words, can include spaces, visible when selecting a field type. e.g. Donate Button
 *
 * @since        7.29
 * @verified     2016.11.10
...
if ( ! class_exists( 'acf_field_lct_zip_code' ) ) :


	class acf_field_lct_zip_code extends acf_field
	{
		public $name;
		public $label;
		public $category;
		...
		function __construct()
		{
			 ...
			 */
			$this->name = zxzu( 'zip_code' );

			/**
			 * label (string) Multiple words, can include spaces, visible when selecting a field type
			 */
			$this->label = __( 'Zip Code (lct)', 'TD_LCT' );
		...
		function validate_value( $valid, $value )
		{
			if (
				$value
				&& strlen( preg_replace( '/[^0-9]/', '', $value ) ) != 5
			) {
				$valid = __( 'Zip code needs to be 5 digits', 'TD_LCT' );
			}
	...
	acf_register_field_type( 'acf_field_lct_zip_code' );
