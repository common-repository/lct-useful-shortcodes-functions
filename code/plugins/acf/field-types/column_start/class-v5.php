<?php
/**
 * `{zxzu}column_start`: Single word, no spaces. Underscores allowed. e.g. donate_button
 * `Column Start`: Multiple words, can include spaces, visible when selecting a field type. e.g. Donate Button
 *
 * @since        7.29
 * @verified     2016.11.10
...
if ( ! class_exists( 'acf_field_lct_column_start' ) ) :


	class acf_field_lct_column_start extends acf_field
	{
		public $name;
		public $label;
		public $category;
		...
		{
			lct_acf_register_field_setting( 'column_start_width' );


			/**
			 * name (string) Single word, no spaces. Underscores allowed
			 */
			$this->name = zxzu( 'column_start' );

			/**
			 * label (string) Multiple words, can include spaces, visible when selecting a field type
			 */
			$this->label = __( 'Column Start', 'TD_LCT' );
			...
			$this->defaults = [
				'label'                          => $this->label,
				get_cnst( 'column_start_width' ) => 50,
			];
		...
		function render_field_settings( $field )
		{
			acf_render_field_setting( $field, [
				...
				'instructions' => '',
				'type'         => 'select',
				'name'         => get_cnst( 'column_start_width' ),
		...
		{
			if ( $field[ get_cnst( 'column_start_width' ) ] ) {
				$width = $field[ get_cnst( 'column_start_width' ) ];
			} else {
				$width = $this->defaults[ get_cnst( 'column_start_width' ) ];
			}


			$data = [
				'data-' . get_cnst( 'column_start_width' ) => $width,
			];
		...
		function input_form_data()
		{
			...
			if ( lct_is_display_form_or_pdf() ) {
				echo '<style>
			.lct_acf_display_form .acf-field-lct-column-start{
		...
		function update_field( $field )
		{
			...
			$field['label'] = $this->defaults['label'] . ': ' . lct_acf_get_column_start_width_label( $field[ get_cnst( 'column_start_width' ) ] );
		...
		{
			$find = sprintf(
				'#<div %s>(.*?)data-%s"(.*?)></span></div></div>#',
				'(.*?)acf-field-' . zxza() . '-column-start(.*?)',
				$this->name
			);
			...
			if ( ! empty( $matches[0] ) ) {
				foreach ( $matches[0] as $match_key => $find ) {
					...
					$width      = $field_data[ 'data-' . get_cnst( 'column_start_width' ) ] - 2;


					$replace = sprintf(
						'<div class="acf-field lct_avoid_page_break_inside"><div class="acf-field acf-field-%s-column-start%s style="%s">',
						zxza(),
						$matches[2][ $match_key ],
			...
			if ( ! empty( $matches[0] ) ) {
				foreach ( $matches[0] as $match_key => $find ) {
					...
					$replace = sprintf(
						'<div class="acf-field lct_avoid_page_break_inside"><div class="acf-field acf-field-%s-column-start%s style="%s">%s<%s>%s<%s>',
						zxza(),
						$matches[2][ $match_key ],
						'width: ' . $width . '%;padding-right: 2%;',
						$matches[5][ $match_key ],
	...
	acf_register_field_type( 'acf_field_lct_column_start' );
